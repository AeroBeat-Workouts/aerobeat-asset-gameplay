#!/usr/bin/env python3
"""One-shot canonical 0.0.8 builder preserving the approved source authority."""
...
GENERATION_MARKER="GENERATE_OK release=0.0.8 assets=7 sources=10 manifests=10 release_files=17 review_pngs=68 review_metadata=5"
...
def prepare(root,stage):
    ...
    (stage/"review").mkdir()
    shutil.copytree(root/"release/raw/0.0.7",stage/"release/raw/0.0.7")
...
def main():
    parser=argparse.ArgumentParser(); parser.add_argument("--root",default="."); parser.add_argument("--release",required=True,choices=[RELEASE]); args=parser.parse_args()
    ...
    prepare(root,stage)
    output=run_checked([blender,"--background","--factory-startup","--python",str(root/"tools/generate.py"),"--","--output-root",str(stage),"--release",RELEASE],operation="single canonical rounded generation",marker=GENERATION_MARKER,postcondition=lambda:(stage/"release/raw"/RELEASE).is_dir() and (stage/"review"/RELEASE).is_dir())
    print(output,end="")
